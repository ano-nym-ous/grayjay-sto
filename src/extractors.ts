...
//
// IMPORTANT: this runs entirely on the RAW fetched HTML string via regex.
// Grayjay's desktop and mobile builds ship different HTML parsers, and the
// mobile parser both mishandles attribute selectors like
// `script[type="application/json"]` (returns null) and mangles the base64 blob
// when re-serializing via `innerHTML`. Staying string-only keeps VOE working
...
export function getVoeStreamUrl(redirectUrl: string): ResolvedStream {
    ...
    // Primary: <script type="application/json">...</script> obfuscated blob.
    // Match on the raw HTML (attribute order / quoting tolerant).
    ...
    // Fallback 3: a plain `"source":"https://.../master.m3u8"` (or 'source')
    // sometimes appears unobfuscated in the page JS.
